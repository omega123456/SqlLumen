...
async function openAiPanel(page: Page) {
  ...
  await expect(aiPanel(page)).toBeVisible({ timeout: APP_READY_MS })
}
...
test.describe('AI Assistant', () => {
  ...
  test('AI panel toggle — enabling AI shows workspace rail, clicking opens panel', async ({
    ...
    await page.getByTestId('ai-sidebar-expand').click()
    await expect(aiPanel(page)).toBeVisible({ timeout: APP_READY_MS })
    await expect(aiPanel(page).getByTestId('ai-panel-header')).toBeVisible()
    await expect(aiPanel(page).getByTestId('ai-chat-messages')).toBeVisible()
    await expect(aiPanel(page).getByTestId('ai-chat-input')).toBeVisible()
  })
  ...
  test('Welcome state — shows welcome message and suggestion chips', async ({ page }) => {
    ...
    await expect(aiPanel(page).getByTestId('ai-welcome-state')).toBeVisible({ timeout: APP_READY_MS })
    ...
    const chips = aiPanel(page).getByTestId('ai-suggestion-chip')
  ...
  test('Sending a message — AI responds with mock streaming', async ({ page }) => {
    ...
    const textarea = aiPanel(page).getByTestId('ai-chat-textarea')
    await textarea.fill('How do I select active users?')
    await aiPanel(page).getByTestId('ai-send-button').click()

    // User message should appear
    await expect(aiPanel(page).getByTestId('ai-message-user')).toBeVisible({ timeout: APP_READY_MS })
    await expect(aiPanel(page).getByTestId('ai-message-user')).toContainText('How do I select active users?')

    // Wait for the AI response to finish streaming
    await expect(aiPanel(page).getByTestId('ai-message-assistant')).toBeVisible({ timeout: APP_READY_MS })
    await expect(aiPanel(page).getByTestId('ai-message-assistant')).toContainText(
      'This query filters for active users',
      { timeout: APP_READY_MS }
    )

    // Welcome state should be gone now
    await expect(aiPanel(page).getByTestId('ai-welcome-state')).toBeHidden()
  })
  ...
  test('Suggestion chips — clicking fills textarea', async ({ page }) => {
    ...
    const firstChip = aiPanel(page).getByTestId('ai-suggestion-chip').first()
  ...
  test('Panel close — clicking X closes the AI panel', async ({ page }) => {
    ...
    await aiPanel(page).getByTestId('ai-close-button').click()
  ...
  test('Clear conversation — resets to welcome state', async ({ page }) => {
    ...
    await aiPanel(page).getByTestId('ai-clear-button').click()

    // Welcome state should return
    await expect(aiPanel(page).getByTestId('ai-welcome-state')).toBeVisible({ timeout: APP_READY_MS })
    await expect(aiPanel(page).getByTestId('ai-message-user')).toBeHidden()
    await expect(aiPanel(page).getByTestId('ai-message-assistant')).toBeHidden()
  })
  ...
  test('Error state — shows error banner when AI endpoint is unreachable', async ({ page }) => {
    ...
    await expect(aiPanel(page).getByTestId('ai-error-banner')).toBeVisible({ timeout: APP_READY_MS })
    await expect(aiPanel(page).getByTestId('ai-error-banner')).toContainText('Connection refused')
  ...
  test('Thinking block — AI response with reasoning shows ThinkingBlock', async ({ page }) => {
    ...
    await expect(aiPanel(page).getByTestId('thinking-block')).toBeVisible({ timeout: APP_READY_MS })
    ...
    await expect(aiPanel(page).getByTestId('thinking-block-header')).toContainText('Reasoning')
  ...
  test('Schema retrieval — DDL contains approximate rows and table comment', async ({ page }) => {
    ...
    await expect(aiPanel(page).getByTestId('ai-message-user')).toContainText('Show me all users')
    await expect(aiPanel(page).getByTestId('ai-error-banner')).toBeHidden()
  })
  ...
  test('Multi-turn conversation — second turn retrieves context from prior turn', async ({
    ...
    await expect(aiPanel(page).getByTestId('ai-message-assistant').first()).toBeVisible({
      timeout: APP_READY_MS,
    })
    await expect(aiPanel(page).getByTestId('ai-message-assistant').first()).toContainText(
      'This query filters for active users',
      { timeout: APP_READY_MS }
    )
    ...
    await expect(aiPanel(page).getByTestId('ai-message-assistant').nth(1)).toBeVisible({
      timeout: APP_READY_MS,
    })
    await expect(aiPanel(page).getByTestId('ai-message-assistant').nth(1)).toContainText(
      'This query filters for active users',
      { timeout: APP_READY_MS }
    )
    ...
    const userMessages = aiPanel(page).getByTestId('ai-message-user')
