...
- **Settings** — general, editor, and results preferences; theme (light / dark / system) persisted locally; current workspace session restored on relaunch and auto-saved on close / every 5 minutes when enabled. The Results settings page includes a "Show table data tabs in bottom panel" toggle (opt-in, off by default) that moves table-data workspace tabs to a compact bottom rail near the data area, keeping the main tab bar focused on query editors and other tab types.
