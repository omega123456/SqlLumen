...
*Sample run (2026-03-31): After **Connect**, `pi_management` appeared in the tree; `permissions` opened via double-click under **Tables**.*
...
- **Opening an editor:** **`webview_interact`** `double-click` at the **center** of the target cell (`getBoundingClientRect()` center in `webview_execute_js` works well). Accessibility tree **`ref=e…`** ids **expire** after DOM updates — prefer **CSS `data-testid`** or **computed x/y**, not stale refs.
...
- **`Tab`** from MCP may move focus to a non-input node; if so, **double-click** the next cell by coordinates and repeat.
...
3. **Nullable fields in NULL mode:** for a new row, **name** / **key** (and sometimes timestamps) may start as **NULL** with the **NULL** toggle active. Click the **`NULL`** button next to the field to **turn NULL off** (toggle to empty / editable text) **before** typing — e.g. **`btn-form-null-name`**, **`btn-form-null-key`**. *Without this step, typing may not register as intended.*
...
After **`double-click`** so `document.activeElement` is the cell text input:
...
*Re-run from §1 whenever you need to validate table-data insert/save behavior through the real app + MCP.*
